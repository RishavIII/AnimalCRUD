...
public interface ElephantRepository extends JpaRepository<Elephant, Long> {
    ...
    @Query(value = "SELECT * FROM elephants e WHERE e.age >= 18", nativeQuery = true)
    List<Elephant> findAdultElephants();

    @Query(value = "SELECT * FROM elephants e WHERE LOWER(e.name) LIKE LOWER(CONCAT('%', :namePart, '%'))", nativeQuery = true)
